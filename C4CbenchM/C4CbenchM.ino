...
const char szPiDigits[] = "314159265358"; // 12 digits ONLY for PI_DIGITS 3
const uint32_t sumPi60dig = 14425;
// DEBUG TEST
...
void c4cBench() {
  ...
  thisNet[0] = (piTime - piCycles / 600); // hold cascade
  piCycles = 0;
  piTime = micros();
  ...
  Serial.printf("\n%s PERF _isr() char[] Test %% Diff versus production T4:\n", szTeensy );
  for ( int ii = 0; ii < 4; ii++ ) {
    Serial.printf("\t_isr() char[] Test @%u us %% Diff of %5.3f\n", usISR[ii], knownISR[ii] / thisISR[ii]);
  ...
  thisNet[1] = (piTime - piCycles / 600); // cascade/direct with ISR
...
FLASHMEM int isEncrypt( bool doPrint ) {
  int ok = 0;
  if ( doPrint) Serial.println( title_text );
  ...
  if ((IOMUXC_GPR_GPR11 & 0x100) == 0x100) {
    if ( doPrint) Serial.println("Pass: Bus Encryption Engine is active");
  } else {
    if ( doPrint) Serial.println("Fail: Bus Encryption Engine is not active");
    ok--;
  }

  uint32_t begin_address = IOMUXC_GPR_GPR18 & ~0x3FF;
  if (begin_address == 0x60001400) {
    if ( doPrint) Serial.println("Pass: Encryption region starts at proper address");
  } else {
    if ( doPrint) Serial.println("Fail: Encryption region starts at wrong address");
    ok--;
  }
  ...
  if (data_end <= end_address) {
    if ( doPrint) Serial.println("Pass: Program data is entirely within encrypted region");
  } else {
    if ( doPrint) Serial.println("Fail: Program data is not within encrypted region");
    ok--;
  }

  uint32_t title_address = ((uint32_t)&isEncrypt) & ~1;
  if (title_address >= begin_address && title_address < end_address) {
    if ( doPrint) Serial.println("Pass: isEncrypt() is within encrypted region");
  } else {
    if ( doPrint) Serial.println("Fail: isEncrypt() is not in encrypted region");
    ok--;
  }

  if ((uint32_t)title_text >= begin_address && (uint32_t)title_text < end_address) {
    if ( doPrint) Serial.println("Pass: title_text[] is within encrypted region");
  } else {
    if ( doPrint) Serial.println("Fail: title_text[] is not in encrypted region");
    ok--;
  }
  ...
  if ((hab_csf[0] & 0xF80000FF) == 0x400000D4) {
    ...
    if ( doPrint) Serial.printf("Pass: CSF is %u bytes with HAB version 4.%u\n", hab_size, hab_minor);
    strcat( szTeensy, " ENC" );
  } else {
    if ( doPrint) Serial.println("Fail: csf not HAB 4.0");
    strcat( szTeensy, " nor" );
    ok--;
  }
  ...
  if (hab_version == 0x40307) {
    if ( doPrint) Serial.println("NOTE: hab_version == 0x40307");
  } else {
    if ( doPrint) Serial.println("NOTE: hab_version NOT == 0x40307");
    ok--;
  }
  if (hab_status == 0xF0) {
    if ( doPrint) Serial.println("NOTE: hab_status == 0xF0");
  } else {
    if ( doPrint) Serial.println("NOTE: hab_status NOT == 0xF0");
    ok--;
  }
  if ((HW_OCOTP_CFG5 & 0x04C00002) == 0x04C00002) {
    if ( doPrint) Serial.print("Secure mode IS set :: Fuses == 0x");
    strcat( szTeensy, " SM:" );
  } else {
    if ( doPrint) Serial.print("Secure mode NOT SET :: Fuses == 0x");
    strcat( szTeensy, " ns:" );
    ok--;
  }
  ...
  sprintf( buf, " @%lu", i );
  strcat( szTeensy, buf );
  if ( doPrint) Serial.println( HW_OCOTP_CFG5, HEX );

  if ( doPrint) Serial.println();
  if (0 == ok && doPrint) Serial.println("All Tests Passed.  :-)");
  else if ( doPrint) Serial.printf(" %d Tests failed.  :-(", -ok);
  return ok;
}
