...
#ifdef USB_MTPDISK
  if ( runOnce && millis() > 10000 ) { // wait for SerMon connect w/MTP
#else
  if ( runOnce && millis() > 1000 ) {
#endif
    notSetup();
    runOnce = false;
    runOnceSer = false;
  }
  if ( runOnceSer && seeSer > 1000 ) {
...
void directoryVerify(File dir, int numTabs, uint32_t numFiles) {
  ...
  if ( numFiles == 0 && 0 != fCnt ) // Dir not '.#' numbered - but file# name matches size
